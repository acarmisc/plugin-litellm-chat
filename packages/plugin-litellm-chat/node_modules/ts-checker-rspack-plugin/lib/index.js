...
(()=>{
    ...
    function resolveTypeScriptGoPackageJsonPath(config) {
        ...
        if (!config.tsgoPackage) throw new Error(`The typescriptPath option must point to "${TYPESCRIPT_PREVIEW_PACKAGE_JSON}" or "${TYPESCRIPT_PACKAGE}@rc".`);
    ...
    async function resolveTypeScriptGoNativeExecutablePath(packageJsonPath, packageName) {
        ...
        if ('function' != typeof getExePath) throw new Error(`Cannot resolve the typescript-go executable from "${packageName}".`);
    ...
    function createTypeScriptGoSupportError(config, error) {
        ...
        if (!isDefaultTypeScriptGoPath(config.typescriptPath)) message.push(`If you set \`typescript.typescriptPath\`, it must be an absolute path to \`${TYPESCRIPT_PACKAGE_JSON}\` from \`${TYPESCRIPT_PACKAGE}@rc\` or \`${TYPESCRIPT_PREVIEW_PACKAGE_JSON}\`.`);
        if (error instanceof Error && error.message) message.push(`Failed to resolve the tsgo executable: ${error.message}`);
        message.push(`You can install it with \`npm add ${TYPESCRIPT_PACKAGE}@rc -D\` or \`npm add ${TYPESCRIPT_PREVIEW_PACKAGE} -D\`.`);
